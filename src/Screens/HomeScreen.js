...
const HomeScreen = () => {
  ...
  const createSession = async () => {
    try {
      const response = await axios.post('localhost:3000/api/session', {
        category,
        cardCount,
        randomize,
      });
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.optionsContainer}>
          <TouchableOpacity style={styles.optionCard} onPress={() => console.log('View previous sessions')}>
...
const styles = StyleSheet.create({
  ...
  card: {
    ...
    flexDirection: 'row', // elements in a row
    justifyContent: "flex-end", // space between elements
  ...
  optionCard: {
    width: '48%', // width relative to screen size
    borderWidth: 1, // border width
    borderColor: '#000', // border color
    borderRadius: 5, // rounded corners
    padding: 30, // padding inside the option card
